...
from polarizedpotentialparticles.custom_conv import CustomNNConv, HNNConv, EHNNConv, PolarizedHNNConv, DistanceHNNConv
...
def deterministic_normal_distribution_batch(num_particles, batch_size, channels, seed=0, device=None):
    generator = torch.Generator(device="cpu")
    generator.manual_seed(seed)
    base_hidden = (torch.randn((num_particles, channels), generator=generator, dtype=torch.float32) - 0.5) * 0.01
    return base_hidden.repeat(batch_size, 1).to(device=device)
...
class PolarizedHamiltonianParticleHC(torch.nn.Module):
    ...
    def initialize_architecture(self):
        self.message_conv = HNNConv(
            out_channels=1 + self.config.particle_config.hidden_dim,
            config=self.config,
        )
    ...
    def update(self, output, x):
        ...
        potential_output = output[:, 0]
        hidden_output = torch.tanh(output[:, 1:])
        dHdx = torch.autograd.grad(
        ...
        dHdx = torch.nan_to_num(dHdx, nan=0.0, posinf=100.0, neginf=-100.0)
        dHdx = torch.clamp(dHdx, -100.0, 100.0)

        x_old = x.clone()
        x_new = x.clone()

        x_new_position = (
            x_new[:, :self.config.N_spatial_dim]
            - dHdx[:, :self.config.N_spatial_dim] * 0.01
            + torch.randn_like(dHdx[:, :self.config.N_spatial_dim]) * self.config.noise_level
        )
        ...
        polarization_tangent = polarization_output - torch.sum(
            polarization_output * polarization_prev,
            dim=1,
            keepdim=True,
        ) * polarization_prev
        
        polarization = F.normalize(
            polarization_prev + self.hidden_step_size * polarization_tangent,
            p=2,
            dim=1,
            eps=1e-8,
        )

        # Keep the non-polarization hidden channels as a direct predicted state-change.
        extra_hidden = extra_hidden_prev + self.hidden_step_size * extra_hidden_output

        if extra_hidden.numel() > 0:
            extra_hidden = torch.clamp(
                extra_hidden,
                -self.hidden_state_clip,
                self.hidden_state_clip,
            )
            x_new_hidden = torch.cat([polarization, extra_hidden], dim=1)
        else:
            x_new_hidden = polarization

        x_new_position = torch.nan_to_num(x_new_position, nan=0.0, posinf=1e3, neginf=-1e3)
        x_new_hidden = torch.nan_to_num(x_new_hidden, nan=0.0, posinf=self.hidden_state_clip, neginf=-self.hidden_state_clip)

        if not torch.isfinite(x_new_position).all() or not torch.isfinite(x_new_hidden).all():
    ...
    def get_initial_state(self):
        ...
        hidden = deterministic_normal_distribution_batch(
            self.config.N_particles,
            batch_size,
            self.config.particle_config.hidden_dim,
            seed=0,
            device=self.device,
        )
        ...
        x[:, self.config.N_spatial_dim:] = hidden
        x.requires_grad_()

        batch = torch.arange(batch_size, device=self.device).repeat_interleave(self.config.N_particles)
        return x, batch

    def forward(self, x, batch, steps, return_history: bool = False, cond: "torch.Tensor | None" = None):
        assert self.message_conv is not None
        ...
        for _ in range(steps):
            ...
            # In concat-conditioning mode, append the conditioning signal to x before
            # the GNN call so the network sees a fresh target cue every step.
            # Gradients flow back through cat to x correctly for Hamiltonian autograd.
            x_in = torch.cat([x, cond], dim=-1) if cond is not None else x

            output = self.message_conv(x_in, edge_index, batch=batch)
            x = self.update(output, x)
...
class DistancePolarizedHamiltonianParticleHC(torch.nn.Module):
    particle_type_name = ParticleType.DISTANCE_POLARIZED_HAMILTONIAN_WITH_HC
    ...
    def initialize_architecture(self):
        self.message_conv = DistanceHNNConv(
            out_channels=1 + self.config.particle_config.hidden_dim,
            config=self.config,
        )
...
class PolarizedHEdgeParticleHC(torch.nn.Module):
    particle_type_name = ParticleType.POLARIZED_EDGE_HAMILTONIAN_WITH_HC
    ...
    def __init__(self, config: Config):
        ...
        if self.config.particle_config.hidden_dim <= 2:
            raise ValueError(
                "PolarizedHEdgeParticleHC requires particle_config.hidden_dim > 2 "
                f"(got {self.config.particle_config.hidden_dim})."
            )
    ...
    def update(self, node_force: torch.Tensor, hidden_output: torch.Tensor, x: torch.Tensor) -> torch.Tensor:
        dHdx = torch.clamp(node_force, -100.0, 100.0)
        hidden_output = torch.tanh(hidden_output)
        ...
        # Angular update: project out the parallel component then renormalize.
        polarization_tangent = polarization_output - torch.sum(
            polarization_output * polarization_prev,
            dim=1,
            keepdim=True,
        ...
        extra_hidden = extra_hidden_prev + self.hidden_step_size * extra_hidden_output
        extra_hidden = torch.clamp(extra_hidden, -self.hidden_state_clip, self.hidden_state_clip)
        ...
        if not torch.isfinite(x_new_position).all() or not torch.isfinite(x_new_hidden).all():
            raise RuntimeError("NaN before cat in PolarizedHEdgeParticleHC.update")
